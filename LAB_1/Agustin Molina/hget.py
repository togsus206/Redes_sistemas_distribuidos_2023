...
def connect_to_server(server_name):
    ...
    # Crear socket
    # COMPLETAR ABAJO DE ESTA LINEA
    # Creamos socket con protocolo IPv4 y de tipo TCP (SOCK_STREAM)
    connect_s = socket.socket(socket.AF_INET, socket.SOCK_STREAM, 0)
    # Aqui deben conectarse al puerto correcto del servidor
    # Establecemos la conexion con la ip del dominio y el puerto 80
    connect_s.connect((ip_address,HTTP_PORT))
    return connect_s
